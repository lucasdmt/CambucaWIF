...
// ---------------------------------------------------------------------------------
// Utilitários para conversão WIF -> chave privada hex
// Usa SHA256 nativo do VanitySearch (_SHA256Initialize + _SHA256Transform)
// ---------------------------------------------------------------------------------
__device__ __forceinline__ int8_t _B58CharToVal(char c) {
...
__device__ void _SHA256Single(const uint8_t* data, int data_len, uint32_t output[8]) {
    uint32_t w[16] = {};
    uint8_t* b = (uint8_t*)w;
    ...
    b[data_len] = 0x80;
    for (int i = 0; i < 16; i++)
        w[i] = __byte_perm(w[i], 0, 0x0123);
    // w[15] recebe o comprimento em bits diretamente, sem bswap,
    // pois é um valor numérico, não bytes lidos da memória.
    w[15] = (uint32_t)(data_len * 8);
    _SHA256Initialize(output);
    _SHA256Transform(output, w);
}

__device__ void _DoubleSHA256Checksum(const uint8_t* data, int data_len, uint8_t checksum[4]) {
    uint32_t h1[8], h2[8];

    _SHA256Single(data, data_len, h1);

    // h1 já sai com bytes em big-endian dentro de cada uint32,
    // pronto para entrar diretamente na segunda rodada.
    uint32_t w2[16] = {};
    for (int i = 0; i < 8; i++)
        w2[i] = h1[i];
    w2[8]  = 0x80000000;  // byte 0x80 no primeiro byte após os 32 bytes
    w2[15] = 0x00000100;  // 256 bits em big-endian

    _SHA256Initialize(h2);
    _SHA256Transform(h2, w2);

    checksum[0] = (h2[0] >> 24) & 0xFF;
    checksum[1] = (h2[0] >> 16) & 0xFF;
    checksum[2] = (h2[0] >>  8) & 0xFF;
    checksum[3] = (h2[0]      ) & 0xFF;
}

__device__ bool _WIFToPrivKey(const char* wif, uint8_t key_out[32]) {

    // --- 1. Comprimento ---
    int len = 0;
    while (wif[len] != '\0' && len < 54) len++;

    if (len != 51 && len != 52)
        return false;

    bool is_compressed = (len == 52);

    // --- 2. Decodificação Base58 ---
    uint8_t decoded[38] = {};
    for (int i = 0; i < len; i++) {
        int8_t val = _B58CharToVal(wif[i]);
        if (val < 0)
            return false;
        int carry = val;
        for (int j = 37; j >= 0; j--) {
            carry += 58 * (int)decoded[j];
            decoded[j] = (uint8_t)(carry & 0xFF);
            carry >>= 8;
        }
    }
    ...
    // --- 3. Localiza o início ---
    int start = 0;
    while (start < 38 && decoded[start] == 0x00) start++;

    if (decoded[start] != 0x80)
        return false;

    // --- 4. Verifica flag de compressão ---
    if (is_compressed && decoded[start + 33] != 0x01)
        return false;

    // --- 5. Verifica checksum ---
    int data_len = is_compressed ? 34 : 33;

    uint8_t checksum_calc[4];
    _DoubleSHA256Checksum(&decoded[start], data_len, checksum_calc);

    const uint8_t* checksum_ref = &decoded[start + data_len];
    for (int i = 0; i < 4; i++) {
        if (checksum_calc[i] != checksum_ref[i])
            return false;
    }

    // --- 6. Extrai os 32 bytes da chave privada ---
    const uint8_t* key_src = &decoded[start + 1];
    for (int i = 0; i < 32; i++)
        key_out[i] = key_src[i];
